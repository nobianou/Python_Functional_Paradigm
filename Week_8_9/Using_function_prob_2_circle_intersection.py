    ...
    while True:
        try:
            ...
            if yi<-radius or yi>radius:
                print()
                print("The line does not intersect the circle, Please enter another value.")
                print()
        except ValueError:
            print("Sorry, enter something valid")
            continue
        if yi<-radius or yi>radius:
            #my_text = Text(Point(0,0), "The line does not intersect the circle")
            #return my_text
            continue
        else:
            break
...
def main():
    ...
    point(x,yi,win)
    text(x,win)
    ex = Text(Point(0,-8),"Click Anywhere to Exit")
    ex.setSize(12)
    ex.draw(win)
    win.getMouse()
    win.close()
